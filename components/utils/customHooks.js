...
export function useAccStep(acc, mag, gyr) {
  ...
  const _algorithm = () => {
    ...
    if (accList[t] > acc_peak_th) {
      cond.peak = true; // added this
      for (let i = -N / 2; i < N / 2; i++) {
        if (i === 0) continue;
        if (accList[t] <= accList[t + i]) { // was >
          cond.peak = false;                // was trues
          break;
        }
      }
    }

    if (cond.peak == true) console.log("peak cond True!");
    ...
    if (Math.max(...diff.prev) > acc_pp_th && Math.max(...diff.next) > acc_pp_th) {
      cond.pp = true;
    }

    if (cond.pp == true) console.log(new Date(), "peak to peak condition true");
    ...
    if (cond.slope == true) console.log("slope cond is TRUE")
...
export function useHeading(acc, mag, gyr) {
  ...
  // Constant declarations
  const dt = 100; // [ms]
  const h_decline = 0;//=(7.5 * Math.PI) / 180;
  ...
  return headingMag.current; // heading
}
